      ...
      {menu?.slider === 'product' && (
        <Col xl={6} className='dropdown-column d-xl-block d-none'>
          <div className='menu-product-slider'>
            <Row>
              {(filteredProduct?.data?.length > 0 ? filteredProduct?.data : productAPIData?.data?.data)?.slice(0, 2)?.map((product, i) => (
                <Col xs={6} key={i}>
                  <ProductBox1 imgUrl={product?.thumbnail?.url} productDetail={{ ...product }} />
                </Col>
              ))}
            </Row>
          </div>
